...
pub async fn verify(data: ProofDataMiden) -> Result<VerificationResult, VerificationError> {
    ...
    let proof_data = match fs::read_to_string(&data.proof_file_path) {
        Ok(x) => x,
        Err(err) => return Err(VerificationError::IOError(err, "Error reading proof file".to_string())),
    };
    let parsed_data: Proof = match serde_json::from_str(&proof_data) {
        Ok(x) => x,
        Err(err) => return Err(VerificationError::JSONError(err, "Error parsing proof JSON".to_string())),
    };
    let verification_result =
        verify_program(&code_frontend, &inputs_frontend, &outputs_frontend, parsed_data.proof);
